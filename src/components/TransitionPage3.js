...
export default function ChooseroundToWaitingRoom() {
  ...
  const initializeLobby = async () => {
    ...
    //   const membersList = await getMembers();
    //   setMembers(membersList);
    //  refreshLobby(); // Ensure to refresh after adding the member
    } catch (error) {
  ...
  useEffect(() => {
    initializeLobby();
    // Add cleanup function if necessary to unsubscribe from events
    return () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.container}>
        ...
        <Text style={styles.emptyMessage}>Ably Call 3 - await addMember  - {members.length} </Text>
      </View>
