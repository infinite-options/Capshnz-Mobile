...
export default function GooglePhotos() {
  ...
  const [request, response, promptAsync] = Google.useAuthRequest({
    ...
    scopes: ["https://www.googleapis.com/auth/userinfo.profile", "https://www.googleapis.com/auth/photoslibrary.readonly"],
  });
  ...
  async function getPhotos(entry) {
    ...
    axios.post(searchGooglePhotosURL, body, { headers }).then((res) => {
  ...
  const submitAlbum = async () => {
    if (albumImages.length < userData.numOfRounds) {
      Alert.alert("Insufficient Images", `Please select an album with enough images for each round.\nTotal Images: ${albumImages.length}\nTotal Rounds: ${userData.numOfRounds}`);
      return;
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        {signedIn ? (
          ...
          <View style={styles.signinContainer}>
            ...
            <TouchableOpacity style={[styles.button, isClicked && styles.buttonClicked]} onPress={() => handlePress()}>
      ...
      <ScrollView style={styles.albumsContainer}>{chooseAlbums()}</ScrollView>
